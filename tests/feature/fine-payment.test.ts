...
jest.mock("@/controllers/librarian/fine-payment.controller", () => ({
  ...
  createFinePayment: jest.fn((req, res) =>
    res.status(201).json({ success: true, data: mockFinePayment, message: "Pembayaran denda berhasil dicatat" })
  ),
  updateFinePayment: jest.fn((req, res) =>
    res.status(200).json({ success: true, data: mockFinePayment, message: "Pembayaran denda berhasil diperbarui" })
  ),
