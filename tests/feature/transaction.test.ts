...
jest.mock("@/controllers/librarian/transaction.controller", () => ({
  ...
  createTransaction: jest.fn((req, res) =>
    res.status(201).json({ success: true, data: mockTransaction, message: "Transaksi berhasil dibuat" })
  ),
  updateTransaction: jest.fn((req, res) =>
    res.status(200).json({ success: true, data: mockTransaction, message: "Transaksi berhasil diperbarui" })
  ),
