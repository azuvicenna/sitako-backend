...
jest.mock("@/controllers/librarian/stack.controller", () => ({
  ...
  showStack: jest.fn((req, res) =>
    res.status(200).json({ success: true, data: mockStack })
  ),
  createStack: jest.fn((req, res) =>
    res.status(201).json({ success: true, data: mockStack, message: "Susunan berhasil ditambahkan" })
  ),
  updateStack: jest.fn((req, res) =>
    res.status(200).json({ success: true, data: mockStack, message: "Susunan berhasil diperbarui" })
  ),
  ...
  describe("GET /api/shelves/:shelfId/stacks/detail/:id", () => {
    it("should return 401 when not authenticated", async () => {
      const res = await request(app).get(
        `/api/shelves/${SHELF_ID}/stacks/detail/${STACK_ID}`
      );
      expect(res.status).toBe(401);
    });
  ...
  describe("PUT /api/shelves/:shelfId/stacks/:id", () => {
    it("should return 401 when not authenticated", async () => {
      const res = await request(app).put(
        `/api/shelves/${SHELF_ID}/stacks/${STACK_ID}`
      );
      expect(res.status).toBe(401);
    });
